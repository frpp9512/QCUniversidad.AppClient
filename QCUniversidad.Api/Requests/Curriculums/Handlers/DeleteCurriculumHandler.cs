...
public class DeleteCurriculumHandler(ICurriculumsManager curriculumsManager) : IRequestHandler<DeleteCurriculumRequest, DeleteCurriculumResponse>
{
    private readonly ICurriculumsManager _curriculumsManager = curriculumsManager;

    public async Task<DeleteCurriculumResponse> Handle(DeleteCurriculumRequest request, CancellationToken cancellationToken)
    {
        try
        {
