var deleteModal = new bootstrap.Modal(document.getElementById('deleteSubjectModal'), { keyboard: true });
requesting = false;

function RiseDeleteModal(id) {
    var subject = GetSubjectName(id);
    SetDeleteModalTextContent("¿Esta seguro que desea eliminar la asignatura '" + subject + "'?");
    var button = document.getElementById("modal-delete-primarybutton");
    button.onclick = function () {
        SendDeleteSubjectRequest(id);
    };
    deleteModal.show();
}

function SendDeleteSubjectRequest(id) {
    if (!requesting) {
        requesting = true;
        HideDeleteModalButtons();
        ShowDeleteModalSpinner();
        var xhttp = new XMLHttpRequest();
        xhttp.open("DELETE", "/subjects/delete?id=" + id, true);
